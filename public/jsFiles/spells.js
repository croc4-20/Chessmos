...
class CreatePlatform extends DefensiveSpell {
  async checkNFTPermission(user) {
    return user.nftAttributes.includes('earth-robe');
  }
...
class FirePawnSpell extends SpecialPawnSpell { //FIRE PAWN IS LINKED TO FIRE STAFF
  constructor() {
    super("Create Fire Pawn");
  }

  async castSpell(squareId, gameBoard) {
    gameBoard.addPiece('FirePawn', squareId);
    // Fire Pawn might have an increased attack range or power.
  }
}
